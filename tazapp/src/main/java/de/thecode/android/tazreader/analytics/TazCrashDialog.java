...
public class TazCrashDialog extends BaseCrashReportDialog implements DialogInterface.OnClickListener {
    ...
    protected void buildAndShowDialog(@Nullable Bundle savedInstanceState) {
        ...
        dialogBuilder.setView(buildCustomView(savedInstanceState))
                     ...
                     .setNeutralButton(getText(R.string.crash_dialog_always_accept), this)
    ...
    protected View buildCustomView(@Nullable Bundle savedInstanceState) {
        ...
        root.setPadding(dpToPx(PADDING), dpToPx(PADDING), dpToPx(PADDING), dpToPx(PADDING));
        ...
        if (comment != null) {
            comment.setPadding(comment.getPaddingLeft(), dpToPx(PADDING), comment.getPaddingRight(), comment.getPaddingBottom());
        ...
        if (email != null) {
            email.setPadding(email.getPaddingLeft(), dpToPx(PADDING), email.getPaddingRight(), email.getPaddingBottom());
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == DialogInterface.BUTTON_POSITIVE || which == DialogInterface.BUTTON_NEUTRAL) {
            ...
            if (which == DialogInterface.BUTTON_NEUTRAL) {
                ...
                prefEditor.putBoolean(ACRA.PREF_ALWAYS_ACCEPT, true);
                PrefUtils.save(prefEditor);
            }

            // Retrieve user comment
            final String comment = userCommentView != null ? userCommentView.getText()
                                                                            .toString() : "";

            // Store the user email
            final String userEmail;
            if (userEmailView != null) {
                userEmail = userEmailView.getText()
                                         .toString();
                final SharedPreferences.Editor prefEditor = prefs.edit();
    ...
    protected void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        if (userEmailView != null && userEmailView.getText() != null) {
            outState.putString(STATE_EMAIL, userEmailView.getText()
                                                         .toString());
        }
    }
