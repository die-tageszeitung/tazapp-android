...
//        if (TazSettings.getInstance(getContext()).getPrefInt(TazSettings.PREFKEY.PAPERMIGRATEFROM, 0) != 0)
//            return;
...
//
//
//        Long currentOpenPaperId = TazSettings.getInstance(getContext()).getPrefLong(TazSettings.PREFKEY.LASTOPENPAPER, -1L);
//        log.debug("+++++++ TazSettings: Current Paper SyncAdapter View: {}", currentOpenPaperId);
//
//        // If ForceSync it's now done
//        TazSettings.getInstance(getContext()).setPref(TazSettings.PREFKEY.FORCESYNC, false);
//
//        // AutoDelete
